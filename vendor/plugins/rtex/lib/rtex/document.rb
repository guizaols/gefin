...
module RTeX
  ...
  class Document
    ...
    def check_path_for(command)
      return command if FileTest.executable?(command)
      return command if (RUBY_PLATFORM.include?('linux') && system_path.split(":").any?{ |path| FileTest.executable?(File.join(path, command))})
      if RUBY_PLATFORM.include?('win32')
        return command if system_path.split(";").any?{ |path| FileTest.executable?(File.join(path, command + '.exe'))}
        path = File.join(RAILS_ROOT + '/../latex/miktex/bin', command + '.exe')
