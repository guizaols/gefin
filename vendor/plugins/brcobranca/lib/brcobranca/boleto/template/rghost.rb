  ...
  module Boleto
    module Template
      ...
      module Rghost
        ...
        def to(name, formato=Brcobranca::Config::OPCOES[:tipo])
          modelo_generico(name, :tipo => formato)
        end
        ...
        #  :tipo => 'pdf'
        def modelo_generico(name, options={})
          ...
          doc.show self.cedente, :tag => :nome_grande
          ...
          doc.render :pdf, :filename => RAILS_ROOT + "/tmp/boletos/#{name}.pdf"
        end
      end
    end
  end
